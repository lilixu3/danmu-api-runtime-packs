	...
	*/
	buildSelection(fields, { isSingleTable = false, table } = {}) {
		...
		for (let i = 0; i < columnsLen; ++i) {
			...
			if ((0, __entity_ts.is)(field, __column_ts.Column)) if (field.columnType === "SQLiteNumericBigInt" || field.columnType === "SQLiteNumeric") if (isSingleTable) chunks.push(field.isAlias ? __sql_sql_ts.sql`cast(${__sql_sql_ts.sql.identifier((0, __alias_ts.getOriginalColumnFromAlias)(field).name)} as text) as ${field}` : __sql_sql_ts.sql`cast(${__sql_sql_ts.sql.identifier(field.name)} as text)`);
